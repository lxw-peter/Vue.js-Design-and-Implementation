function createRenderer(options) {
  ...
  function patchElement(n1, n2) {
    ...
    if (n2.patchFlags) {
      if (n2.patchFlags === 2) {
        //  只更新class
        patchProps(oldProps.class, newProps.class, 'class', el);
      } else if (n2.patchFlags === 3) {
        // 只更新 style
        patchProps(oldProps.style, newProps.style, 'style', el);
  ...
  function patchBlockChildren(n1, n2) {
    for (let i = 0; i < n2.dynamicChildren.length; i++) {
      patchElement(n1.dynamicChildren[i], n2.dynamicChildren[2]);
    }
  }
